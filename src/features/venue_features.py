...
def _compute_venue_stats() -> dict:
    ...
    # Read only necessary columns — handle both original and cricsheet formats
    df = pd.read_csv(IPL_CSV, low_memory=False)
    
    # Compute runs_total if not present
    if "runs_total" not in df.columns:
        runs_bat = pd.to_numeric(df["runs_batter"] if "runs_batter" in df.columns else df.get("runs_off_bat", pd.Series(0, index=df.index)), errors="coerce").fillna(0)
        extras = pd.to_numeric(df["extras"], errors="coerce").fillna(0) if "extras" in df.columns else pd.Series(0, index=df.index)
        df["runs_total"] = runs_bat + extras
...
def get_recent_venue_avg_score(matches: pd.DataFrame, venue: str, before_idx: int, k=5) -> float:
    """Rolling average of the first innings score for the last K matches at this venue."""
    past = matches.iloc[:before_idx]
    venue_matches = past[past["venue"] == venue].tail(k)
    if len(venue_matches) == 0:
        return get_venue_avg_score(venue)
    # Since matches_df might not have 'runs_total' or 'first_innings_score', we might have to approximate 
    # based on the overall matches.csv if it doesn't exist. Wait, matches.csv doesn't have runs scored!
    # If matches.csv doesn't have runs scored, we just fallback or return static for now.
    # We will just return the static score for now as a fallback if runs aren't present.
    if "first_innings_score" in venue_matches.columns:
        raw = venue_matches["first_innings_score"].mean()
        return max(0, min(1, (raw - 120) / (200 - 120)))
    return get_venue_avg_score(venue)


def get_recent_venue_toss_impact(matches: pd.DataFrame, venue: str, before_idx: int, k=10) -> float:
    """Rolling average toss win rate for the last K matches at this venue."""
    past = matches.iloc[:before_idx]
    venue_matches = past[past["venue"] == venue].tail(k)
    if len(venue_matches) < 3:
        return get_venue_toss_impact(venue)
    wins = (venue_matches["toss_winner"] == venue_matches["winner"]).sum()
